...
export default function GovernmentCredentialCard({ 
  credentialType, country, name, dob, issueDate, expiryDate, passportNumber, nationalIdNumber, gradient, status 
}: GovernmentCredentialCardProps) {
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          <div className="flex flex-col gap-0.5">
            <span className="text-[10px] font-bold uppercase tracking-wider opacity-70">{country} &bull; {credentialType}</span>
            <p className="text-base font-bold font-headline leading-tight">{name}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[11px] opacity-90 border-t border-white/10 pt-2.5">
          ...
          <div className="flex items-center gap-1.5 truncate">
            ...
            <span className="truncate">{passportNumber ? `Passport: ${passportNumber}` : `ID: ${nationalIdNumber}`}</span>
          </div>
          <div className="flex items-center gap-1.5 col-span-2 text-[10px] opacity-70">
            <Shield className="h-3 w-3" />
            <span>Valid: {format(new Date(issueDate), "MMM yy")} - {format(new Date(expiryDate), "MMM yy")}</span>
          </div>
        </div>
      </CardContent>
    </Card>
